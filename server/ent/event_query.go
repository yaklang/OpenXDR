...
func (_q *EventQuery) QueryAsset() *AssetQuery {
...
func (_q *EventQuery) QueryAlerts() *AlertQuery {
...
func (_q *EventQuery) WithAsset(opts ...func(*AssetQuery)) *EventQuery {
	query := (&AssetClient{config: _q.config}).Query()
...
func (_q *EventQuery) WithAlerts(opts ...func(*AlertQuery)) *EventQuery {
	query := (&AlertClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := _q.withAsset; query != nil {
		if err := _q.loadAsset(ctx, query, nodes, nil,
			func(n *Event, e *Asset) { n.Edges.Asset = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAlerts; query != nil {
		if err := _q.loadAlerts(ctx, query, nodes,
			func(n *Event) { n.Edges.Alerts = []*Alert{} },
			func(n *Event, e *Alert) { n.Edges.Alerts = append(n.Edges.Alerts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EventQuery) loadAsset(ctx context.Context, query *AssetQuery, nodes []*Event, init func(*Event), assign func(*Event, *Asset)) error {
...
func (_q *EventQuery) loadAlerts(ctx context.Context, query *AlertQuery, nodes []*Event, init func(*Event), assign func(*Event, *Alert)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(alert.FieldEventID)
	}
	query.Where(predicate.Alert(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.AlertsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *EventQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAsset != nil {
			_spec.Node.AddColumnOnce(event.FieldAssetID)
		}
	}

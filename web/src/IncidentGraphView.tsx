...
const COLUMNS = ['asset', 'process', 'alert'] as const
const COL_X: Record<string, number> = { asset: 20, process: 250, alert: 480 }
const NODE_W = 190
const NODE_H = 34
const GAP_Y = 52
...
export function IncidentGraphView({ graph }: { graph: Graph }) {
  const columns: Record<string, GraphNode[]> = { asset: [], process: [], alert: [] }
  for (const node of graph.nodes) (columns[node.type] ?? columns.process).push(node)

  const pos = new Map<string, { x: number; y: number }>()
  for (const col of COLUMNS)
    columns[col].forEach((n, i) => pos.set(n.id, { x: COL_X[col], y: 20 + i * GAP_Y }))

  const rows = Math.max(...COLUMNS.map(c => columns[c].length), 1)
